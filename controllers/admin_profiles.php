...
class Admin_profiles extends Admin_Controller
{
	...
	/*
		Get a sample of the array. Helpful to determinate the Loop Path in the step 2 of creation of a profile ! 
	*/
	...
	public function edit($id)
	{
		...
		$this->_tabs = array(
			array(
				...
				'id'		=> 'recipients',
				'fields'	=> array('profile_name','profile_slug','example_file','eol','delimiter','enclosure','stream_identifier','unzip','datasource','source_format'),
				),
			array(
	 ...
	 */
	public function run($profile_id, $file_id=null)
	{
		//if we do have a file to import
		if ( isset($_POST['file_id']) )
		{
			redirect('/admin/streams_import/profiles/run/'.$profile_id.'/'.$_POST['file_id']);
		}
		elseif (!empty($file_id)&&!empty($profile_id)) {
